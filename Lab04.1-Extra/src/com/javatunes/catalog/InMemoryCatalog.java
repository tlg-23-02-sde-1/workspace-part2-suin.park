...
public class InMemoryCatalog implements Catalog {
    ...
    @Override
    public MusicItem findById(Long id) {  // let's say a 12came in here
        // declare return value
        MusicItem result = null;
        // iterate over 'catalogData', asking each one if it has the provided id.
    ...
    public Collection<MusicItem> findByKeyword(String keyword) {
        ...
        for (MusicItem item : catalogData) {
            ...
            if (title.contains(keyword.toLowerCase()) || artist.contains(keyword.toLowerCase())) {
                result.add(item);
            }
        }
        return result;
    ...
    @Override
    public Collection<MusicItem> findByCategory(MusicCategory category) { // say its POP
    // declare return value, a holding area for matches
       Collection<MusicItem> result = new ArrayList<>();
        for(MusicItem item : catalogData) {
    ...
    /**
     * TASK: find all "rock" items whose price is less than or equal to the specified price. // client speicify price so arg
     */
    ...
    */
    ...
    /**
     * TASK: are all items priced at least $10? (not hardcode it, client will specify)
